...
1. ~~**AppKit image path missing Y-flip.**~~ **Verified correct, not a bug.** Re-audited during the preview-quality pass (2026-09): `MermaidImageRenderer._renderPrepared` (`ImageRenderer.swift:155-159`) and `_renderPreparedFitted` (`ImageRenderer.swift:219-223`) both apply `translateBy(x: 0, y: diagBounds.height); scaleBy(x: 1, y: -1)` on the AppKit branch, matching `MermaidView`/`MermaidLayer`. The actual mirrored/rotated-180 bug this note was chasing was in Penumbra's own `MarkdownPreviewCGRenderer`, not the vendor tree — see `MarkdownPreviewCGRenderer.drawImage`'s fix and the `testMermaidImageOrientationNotMirrored` / `testMermaidVendorRasterOrientationIsUpright` regression tests in `MarkdownPreviewTests.swift`, which pin the vendor's flip so a future re-sync can't silently drop it.
2. **Deprecated `NSImage.lockFocus()`** in `MermaidLayer.renderImage`. **Fix:** draw into a `CGContext` bitmap (as `ImageRenderer` already does on AppKit) so Retina / color space match Metal uploads. (Not on Penumbra's paint path — the preview renders through `MermaidPaintAdapter.rasterize`, not `MermaidLayer`.)
