...
/// Bridges a Penumbra `TextView` to the Editor Intelligence Platform.
///
...
public final class PenumbraEditorAdapter: EditorAdapter, @unchecked Sendable {
    ...
    public func applyEdit(_ edit: TextEdit, toDocumentWithID documentID: DocumentID) async throws {
        guard documentID == self.documentID else {
            throw PenumbraEditorAdapterError.unknownDocumentID(documentID)
        }
        guard let textView = textView else {
            throw PenumbraEditorAdapterError.textViewDeallocated
        }
        await MainActor.run {
    ...
    private func refreshDocument() {
        ...
        refreshTask = Task { @MainActor in
            ...
            PenumbraSignposts.interval("PenumbraEditorAdapter.refreshDocument") {
                if edits.isEmpty {
...
extension PenumbraEditorAdapter: TextViewDelegate {
...
public enum PenumbraEditorAdapterError: Error {
