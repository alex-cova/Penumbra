...
10. **Gradle model cache for fast reopen.** After a successful sync, Umbra stores the decoded `JavaGradleProjectModel` under `~/Library/Application Support/com.umbra.editor/gradle-models/<sha256(projectPath)>/` alongside a `meta.json` fingerprint of every Gradle build-input file (`settings.gradle`, `build.gradle(.kts)`, `gradle.properties`, `gradle/libs.versions.toml`, `gradle/wrapper/gradle-wrapper.properties`, and subproject scripts). On reopen, if the fingerprint and init-script `formatVersion` still match, Umbra wires up the already-cached per-JAR and per-source shards immediately (skipping the whole-tree fallback) and marks the project synced, then runs Gradle silently in the background to refresh the model. Manual Reload invalidates the cache first. Individual JAR/source shards remain in `~/Library/Caches/.../JavaIndex/v2/` with stamp-based invalidation as before.
