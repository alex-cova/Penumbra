...
struct HSplit<P: View, D: SplitDivider, S: View>: View {
    ...
    init(@ViewBuilder left: @escaping () -> P, @ViewBuilder right: @escaping () -> S) where D == Splitter {
    ...
    func splitter<T>(@ViewBuilder _ splitter: @escaping () -> T) -> HSplit<P, T, S> where T: View {
        HSplit<P, T, S>(fraction: fraction, hide: hide, constraints: constraints, onDrag: onDrag, primary: { primary }, splitter: splitter, secondary: { secondary })
    }

    /// Return a new instance of HSplit with `constraints` set to these values.
    func constraints(minPFraction: CGFloat? = nil, minSFraction: CGFloat? = nil, priority: SplitSide? = nil, dragToHideP: Bool = false, dragToHideS: Bool = false) -> HSplit {
    ...
    func onDrag(_ callback: ((CGFloat) -> Void)?) -> HSplit {
        HSplit(fraction: fraction, hide: hide, constraints: constraints, onDrag: callback, primary: { primary }, splitter: { splitter }, secondary: { secondary })
    }

    /// Return a new instance of HSplit with its `splitter.styling` set to these values.
    func styling(color: Color? = nil, inset: CGFloat? = nil, visibleThickness: CGFloat? = nil, invisibleThickness: CGFloat? = nil, hideSplitter: Bool = false) -> HSplit {
    ...
    func fraction(_ fraction: FractionHolder) -> HSplit<P, D, S> {
    ...
    func hide(_ side: SplitSide) -> HSplit<P, D, S> {
